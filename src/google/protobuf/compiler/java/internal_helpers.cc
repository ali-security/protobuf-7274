...
void GenerateLarge(io::Printer* printer, const EnumDescriptor* descriptor,
  ...
  printer->Emit(
      {{"classname", descriptor->name()},
       ...
       {"values_func",
        [&] {
          printer->Emit({{"size", descriptor->value_count() + 1}},
                        R"(
              int ordinal = 0;
          ...
          printer->Emit({{"last_index", descriptor->value_count()}}, R"(
      ...
      R"(
        ...
        public$static$final class $classname$
          ...
          public static final $classname$ UNRECOGNIZED = new $classname$(-1, $value_count$, "UNRECOGNIZED");
            ...
            printer->Emit(
                {{"cases",
                  [&] {
                    for (int i = start; i < end; i++) {
                      const EnumValueDescriptor* value = descriptor->value(i);
                      // Only support string lookup for non-aliases. This is
